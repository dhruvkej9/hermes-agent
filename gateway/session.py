...
class SessionStore:
    ...
    def _resolve_session_isolation(
        ...
        """
        return resolve_session_isolation_flags(self.config, source.platform)
...
def resolve_session_isolation_flags(config: GatewayConfig, platform: str) -> tuple[bool, bool]:
    """Resolve per-platform isolation overrides for *platform*.

    Module-level twin of GatewaySessionManager._resolve_session_isolation so
    every consumer (session keys, write-path attribution, session context)
    reads the same flags from the same source: per-platform
    ``extra.group_sessions_per_user`` / ``extra.thread_sessions_per_user``
    win when present, falling back to the global gateway config.
    """
    default_group = getattr(config, "group_sessions_per_user", True)
    default_thread = getattr(config, "thread_sessions_per_user", False)
...
def build_session_context(
    ...
    context = SessionContext(
        ...
        shared_multi_user_session=is_shared_multi_user_session(
            source,
            group_sessions_per_user=resolve_session_isolation_flags(config, source.platform)[0],
            thread_sessions_per_user=resolve_session_isolation_flags(config, source.platform)[1],
        ),
    )
